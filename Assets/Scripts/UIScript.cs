...
[AddComponentMenu("Kaitsu/Extras/UIScript")]
public class UIScript : MonoBehaviour
{
	...
	public Text rightLabel, leftLabel, nameLabel, enemies;
	public Text nimi;
	public Text enemiesNr;
	string nimiString = "Pelaajan nimi";
	...
	private void Start()
	{
		...
		{
            // No setup needed
            scores[0] = GameManager.score;
            nimiString = GameManager.playerName;
            nimi.text = nimiString;
        }
		else
		{
	...
	public void AddPoints(int playerNumber, int amount = 1)
	{
		...
		if(numberOfPlayers == Players.OnePlayer)
		{
			...
			GameManager.score++;
			enemiesNr.text = GameManager.enemiesNumber.ToString();
		}
		else
		{
	...
	public void GameWon(int playerNumber)
	{
	    ...
	    if (!gameOver)
	    {
			//gameOver = true;
			GameOver(0);
			winLabel.text = "You have won with  " +scores[0] + " scores!";
